...
const EmployerJobs = ({ refreshTrigger }) => {
    ...
    const fetchEmployerJobs = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/jobs/v1/employerjobs",
          {
            headers: {
  ...
  const deleteJob = async (jobId) => {
    try {
      await axios.delete(
        `http://localhost:5000/api/jobs/v1/deletejob/${jobId}`,
        {
          headers: {
            "auth-token": localStorage.getItem("auth-token"),
          },
  ...
  const saveJob = async (jobId) => {
    try {
      await axios.put(
        `http://localhost:5000/api/jobs/v1/updatejob/${jobId}`,
        formData,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const handleStatusUpdate = async (jobId, userId, status) => {
    try {
      await axios.put(
        `http://localhost:5000/api/jobs/v1/updatejobstatus/${jobId}/${userId}`,
        { status },
        {
          headers: {
            "Content-Type": "application/json",
